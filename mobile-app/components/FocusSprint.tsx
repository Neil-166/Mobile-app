...
export default function FocusSprint({ compact = false }: FocusSprintProps) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-[#27272F] bg-[#16161D] p-3">
        <div className="relative h-11 w-11 shrink-0">
          <svg className="h-11 w-11 -rotate-90" viewBox="0 0 44 44" aria-hidden="true">
            ...
            <circle
              ...
              r="18"
              fill="none"
              stroke={state === 'completed' ? '#22C55E' : state === 'running' ? '#8B5CF6' : 'rgba(255,255,255,0.18)'}
              strokeWidth="3"
              strokeLinecap="round"
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[11px] text-[#8B8B99]">{sessionLabel}</p>
        ...
        <div className="flex gap-1.5">
            ...
            <>
              {state === 'running' ? (
                <button
                  type="button"
                  onClick={handlePause}
                  className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-white transition-colors hover:bg-white/10"
                ...
                <button
                  type="button"
                  onClick={handleStart}
                  className="grid h-9 w-9 place-items-center rounded-lg bg-[#8B5CF6]/20 text-[#8B5CF6] transition-colors hover:bg-[#8B5CF6]/30"
              ...
              <button
                type="button"
                onClick={handleReset}
                className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-[#8B8B99] transition-colors hover:bg-white/10"
          ...
          {state === 'completed' && (
            <button
              type="button"
              onClick={handleReset}
              className="grid h-9 w-9 place-items-center rounded-lg bg-[#22C55E]/15 text-[#22C55E]"
    ...
    <div className="relative">
      <AnimatePresence>
        {showCelebration && (
          <motion.div
            ...
            className="absolute inset-0 z-10 flex flex-col items-center justify-center rounded-2xl border border-[#22C55E]/30 bg-[#16161D] p-6"
          >
            <motion.div
              initial={{ scale: 0.5, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ type: 'spring', stiffness: 300, damping: 16 }}
              className="grid h-16 w-16 place-items-center rounded-full bg-[#22C55E]/15"
            >
              <svg viewBox="0 0 52 52" className="h-9 w-9" aria-hidden="true">
                <circle cx="26" cy="26" r="24" fill="none" stroke="#22C55E" strokeOpacity="0.25" strokeWidth="4" />
                <path
            ...
            <p className="mt-3 text-lg font-bold text-white">Sprint complete</p>
            <p className="mt-1 text-sm text-[#C7C7D1]">25 minutes of focus. You earned a real break.</p>
      ...
      <div className="rounded-2xl border border-[#27272F] bg-[#16161D] p-5">
        <div className="mb-5 flex items-center gap-2">
          ...
          <span className="ml-auto inline-flex items-center gap-1 rounded-full border border-[#F59E0B]/20 bg-[#F59E0B]/10 px-2 py-0.5 text-[10px] font-semibold text-[#F59E0B]">
        ...
        <div className="flex flex-col items-center">
          <div className="relative mb-5 h-40 w-40">
            <svg className="h-40 w-40 -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
              ...
              <motion.circle
                ...
                r="40"
                fill="none"
                stroke={state === 'completed' ? '#22C55E' : 'url(#sprint-grad)'}
                strokeWidth="7"
                strokeLinecap="round"
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="mt-1 text-xs text-[#8B8B99]">
            ...
            {state !== 'completed' && (
              <>
                {state === 'running' ? (
                  ...
                  <Button onClick={handleStart} size="sm" id="start-sprint" className="bg-[#8B5CF6] text-white">
          ...
          {state === 'idle' && (
            <p className="mt-3 text-center text-xs leading-relaxed text-[#8B8B99]">
