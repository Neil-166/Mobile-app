...
function BuildPreview() {
  return (
    <section aria-labelledby="build-preview-title" className="overflow-hidden rounded-2xl border border-[#27272A] bg-[#16161A]">
      <div className="flex items-center justify-between border-b border-[#27272A] bg-white/[0.035] px-4 py-3">
        ...
        <span className="font-mono text-[10px] text-zinc-600">portfolio-card.html</span>
      </div>
      <div className="relative min-h-48 overflow-hidden bg-[#0d0d10] p-5">
        ...
        <div className="relative mx-auto max-w-60 rounded-2xl border border-white/10 bg-[#17171d] p-4 shadow-2xl shadow-black/40">
          <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-[#8B5CF6] to-[#22D3EE] text-xs font-bold text-white">AS</div><div><p className="text-xs font-bold text-white">Arjun Sharma</p><p className="text-[10px] text-zinc-500">Frontend builder · Delhi</p></div></div>
          <p className="mt-3 text-[10px] leading-relaxed text-zinc-400">I turn curious ideas into calm, useful interfaces.</p>
          <div className="mt-3 flex gap-1.5"><span className="rounded-md bg-[#8B5CF6]/15 px-1.5 py-1 text-[9px] text-[#A78BFA]">React</span><span className="rounded-md bg-[#22D3EE]/10 px-1.5 py-1 text-[9px] text-[#67E8F9]">CSS</span><span className="rounded-md bg-white/5 px-1.5 py-1 text-[9px] text-zinc-400">Motion</span></div>
...
export default function DayChallengePage() {
  ...
  const handleCopyLinkedIn = async () => {
    ...
    toast.success('LinkedIn draft copied — make it sound like you.');
  };
  ...
  const generateReflection = () => {
    ...
    toast('A reflection starter is ready. Edit it in your voice.');
  };
  ...
  const handleScreenshot = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    toast.success('Screenshot attached to this local submission.');
  };
  ...
  const submit = async () => {
    ...
    if (!allRequirementsMet) {
      setActiveTab('brief');
      toast.error('Finish the requirements first — you are close.');
      return;
    }
    if (!githubUrl) {
      setActiveTab('submit');
      toast.error('Add your GitHub repository URL before submitting.');
      return;
    }
    ...
    toast.success('Day 12 is on your public record. Streak +1 🔥');
  };
  ...
  return (
    <div className="min-h-screen bg-[#09090B] pb-32">
      ...
      <header className="sticky top-0 z-40 border-b border-[#27272A] glass safe-top"><div className="mx-auto flex max-w-lg items-center justify-between px-5 py-3"><div className="flex min-w-0 items-center gap-3"><button type="button" onClick={backToDashboard} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/5 text-white transition-colors hover:bg-white/10" aria-label="Go back"><ArrowLeft className="h-5 w-5" /></button><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#A78BFA]">Day {challenge.day} of 60</p><h1 className="truncate text-sm font-bold text-white">{challenge.title}</h1></div></div><Badge variant={isSuccess ? 'success' : 'outline'}>{isSuccess ? 'Submitted' : 'Open'}</Badge></div></header>
      <nav className="sticky top-[64px] z-30 border-b border-[#27272A] bg-[#09090B]/95 px-5 backdrop-blur" aria-label="Challenge sections"><div className="mx-auto flex max-w-lg">{(['brief', 'workflow', 'submit'] as Tab[]).map((tab) => <button key={tab} type="button" onClick={() => setActiveTab(tab)} className={`min-h-12 flex-1 border-b-2 text-xs font-semibold capitalize transition-colors ${activeTab === tab ? 'border-[#8B5CF6] text-[#A78BFA]' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`} aria-current={activeTab === tab ? 'page' : undefined}>{tab}</button>)}</div></nav>
      <main className="mx-auto max-w-lg px-5 pt-6">
        {isSuccess && <motion.section initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="mb-6 overflow-hidden rounded-2xl border border-[#22C55E]/30 bg-[#22C55E]/[0.07] p-4"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#22C55E]/15"><CheckCircle2 className="h-5 w-5 text-[#4ADE80]" /></span><div><p className="text-sm font-bold text-white">Day 12 is on your record.</p><p className="mt-1 text-xs leading-relaxed text-zinc-400">Streak protected for today 🎉 Your small, public proof matters.</p></div></div><div className="mt-4"><MomentumCard day={challenge.day} streak={currentStudent.streak + 1} projectName={challenge.title} githubUrl={githubUrl} /></div></motion.section>}
        <AnimatePresence mode="wait">{tabContent}</AnimatePresence>
      </main>
      {!isSuccess && <div className="fixed bottom-0 left-0 right-0 z-40 bg-gradient-to-t from-[#09090B] via-[#09090B] to-transparent px-5 pb-[calc(env(safe-area-inset-bottom)+16px)] pt-8"><div className="mx-auto max-w-lg"><Button type="button" onClick={() => activeTab === 'submit' ? submit() : setActiveTab('submit')} disabled={activeTab === 'submit' && !submissionReady} size="lg" className="w-full shadow-xl shadow-purple-900/30">{isSubmitting ? <><Loader2 className="h-5 w-5 animate-spin" />Submitting…</> : <><Send className="h-5 w-5" />{submitLabel}</>}</Button></div></div>}
    </div>
  );
}
